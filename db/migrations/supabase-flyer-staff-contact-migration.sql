-- ============================================================
-- Flyer contact card: show the person who shared the flyer.
--
-- Staff-generated flyer links now carry the staff member's id
-- (#flyer/<stockId>/<partnerId>?staff=<staffId>); this RPC resolves
-- their name + email so the public flyer's "Speak with" card shows
-- the person the client actually dealt with, not the firm owner.
-- Falls back to the firm owner when no staff id is supplied
-- (owner-shared links and all links minted before this change).
--
-- partner_staff carries no phone column, so the card's Call button
-- keeps the firm's number in both cases (client-side behaviour).
--
-- Signature change (new optional p_staff_id): drop the old 2-arg
-- version first — keeping both would leave PostgREST an ambiguous
-- overload for 2-argument calls.
-- ============================================================

DROP FUNCTION IF EXISTS public.get_flyer_payload(text, uuid);
...
-- Public flyer is fetched with the anon key; re-grant after the drop.
GRANT EXECUTE ON FUNCTION public.get_flyer_payload(text, uuid, uuid) TO anon, authenticated;
